...
use Molajo\Controller\Api\DeleteControllerInterface;
    ...
    public function getDeleteData()
    {
        ...
        if (isset($this->row->$primary_key)) {
            $this->model->query->where(
                $this->model->database->qn($primary_prefix) . '.' . $this->model->database->qn($primary_key)
                . ' = ' . $this->model->database->q($this->row->$primary_key)
            );
        } elseif (isset($this->row->$name_key)) {
            $this->model->query->where(
                $this->model->database->qn($primary_prefix) . '.' . $this->model->database->qn($name_key)
                . ' = ' . $this->model->database->q($this->row->$name_key)
            );
        } else {
        ...
        if (isset($this->row->catalog_type_id)) {
            $this->model->query->where(
                ...
                . '.' . $this->model->database->qn('catalog_type_id')
                . ' = ' . $this->model->database->q($this->row->catalog_type_id)
            );
        }
...
//		echo '<br /><br /><br />';
    ...
    protected function verifyPermissions()
    {
        ...
        //$results = $this->permissions->verifyActionPermissions('Delete', $this->row->catalog_id);
        //if ($results === false) {
        //error
        //return false (not yet)
        //}
    ...
    protected function onBeforeDeleteEvent()
    {
        ...
        $arguments = $this->event->scheduleEvent('onBeforeDelete', $arguments, $this->plugins);
        if ($arguments === false) {
