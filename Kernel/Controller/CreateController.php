...
use Exception;
use Molajo\Controller\Api\CreateControllerInterface;
use Molajo\Controller\Exception\CreateControllerException;
...
class CreateController extends ReadController implements CreateControllerInterface
{
    ...
    protected function verifyPermissions()
    {
        ...
        if (isset($this->row->primary_category_id)) {
            $results = $this->permissions->verifyActionPermissions('Create', $this->row->primary_category_id);
            if ($results === true) {
                return true;
            }
        }

        $results = $this->permissions->verifyActionPermissions('Create', $this->row->catalog_type_id);
        if ($results === false) {
            //error
            //return false (not yet)
        }
    ...
    protected function checkFields()
    {
        ...
        $userHTMLFilter = $this->permissions->isUserAuthorisedNoFiltering($this->user->groups);
    ...
    protected function checkForeignKeys()
    {
        ...
        foreach ($foreignkeys as $fk) {
            ...
            if (isset($this->row->$name)) {
                ...
                $controller->model->query->from($controller->model->database->qn($controller->get('table_name')));
    ...
    protected function onBeforeCreateEvent()
    {
        ...
        $arguments = $this->event->scheduleEvent('onBeforeCreate', $arguments, $this->plugins);
    ...
    protected function onAfterCreateEvent($data)
    {
        ...
        $arguments = $this->event->scheduleEvent('onAfterCreate', $arguments, $this->plugins);
