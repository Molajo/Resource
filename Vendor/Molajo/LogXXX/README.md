...
Simple, uniform File and Directory Services API for PHP applications enabling interaction with multiple Log types
...
**Log** provides a common API to read, list, write, rename, delete, copy or move files and folders
...
to fileservices for application support services, like Log, Logging, Log, and Log.

## Basic Usage ##

Each **Log** command shares the same syntax and the same four parameters:

### Log Request ###

```php
    $adapter = new Molajo/Log/Adapter($action, $path, $filesystem_type, $options);
```
#### Parameters ####
...
Metadata is defined in the [Molajo\Log\Properties](https://github.com/Molajo/Log/blob/master/src/Molajo/Log/Type/LogType.php) class. The Properties
class can be extended and customized, as needed, by Log.
...
```php
    $adapter = new \Molajo\Log\Adapter('Read', 'location/of/file.txt');
    echo $adapter->fs->data;
```
...
```php
    ...
    $adapter = new \Molajo\Log\Adapter('List', 'directory-name', $options);
...
```php
    ...
    $adapter      = new \Molajo\Log\Adapter('Write', 'standard', $options, 'Log');
```
...
```php
    ...
    $adapter = new \Molajo\Log\Adapter('Copy', 'name/of/source/file', $options);
```
...
```php
    ...
    $adapter = new \Molajo\Log\Adapter('Move', 'current/folder/year2012', $options);
```
...
```php
    $adapter = new \Molajo\Log\Adapter('Delete', 'name/of/source/folder');
```
...
```php
    $adapter = new \Molajo\Log\Adapter('List', '/first/location');
    $data1   = $adapter->fs->data;

    $adapter = new \Molajo\Log\Adapter('List', '/second/location');
...
```php
    ...
    $adapter = new \Molajo\Log\File($options);
    $data    = $adapter->backup ();
```
...
This instructs PHP to use Composer’s autoloader for **Log** project dependencies.
...
Register Molajo\Log\ subfolder in your autoload process.
...
More Information
----------------
- [Usage](/Log/doc/usage.md)
- [Extend](/Log/doc/extend.md)
- [Specifications](/Log/doc/specifications.md)
