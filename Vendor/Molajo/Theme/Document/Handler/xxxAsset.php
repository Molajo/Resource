 ...
 */
namespace Molajo\Theme\Asset;


use stdClass;
use files;
use Exception;
use Molajo\Theme\Asset\Exception\AssetException;
...
class bkAsset
{
     ...
     * @since  1.0
     */
    protected $links_priorities = array();
     ...
     *
     * $this->assets->addLink(
     *   $url = BASE_FOLDER . '/Application/Extension' . '/' . THEME_URL
     *      . '/' . $this->registry->get('parameters', 'theme_path_node')
    ...
    public function setPriority($type, $url, $priority)
    {
        ...
        if ($update === true) {
            ...
            $priorityHandler = $type . '_priorities';
            ...
            $this->set($priorityHandler, $priorities);
        }
     ...
     * @since   1.0
     */
    public function removeAsset($type, $url)
    {
        $temp = $this->get($type);
        if (is_array($temp) && count($temp) > 0) {
        } else {
    ...
    {
        $priorityHandler = $type . 'Priorities';
        ...
        $priorities = $this->get($priorityHandler);
        sort($priorities);
